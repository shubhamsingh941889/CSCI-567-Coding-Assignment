...
class relu:
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        temp_x = np.where(X <= 0, 0, 1)
        backward_output = np.multiply(grad,temp_x)
        return backward_output
...
class tanh:
    ...
    def backward(self, X, grad):
        ...
        backward_output = np.multiply(np.subtract(1, np.power(np.tanh(X), 2)), grad)
        return backward_output
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha <= 0.0:
                    ...
                    ###################################################################################################
                    momentum[str(module_name + '_' + key)] = np.subtract(np.multiply(_alpha, momentum[str(module_name + '_' + key)]), np.multiply(_learning_rate, g))
...
def main(main_params):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            ######################################################################################
            my_d1 = model['L2'].backward(d1, grad_a2)
            my_h1 = model['drop1'].backward(h1, my_d1)
            my_a1 = model['nonlinear1'].backward(a1, my_h1)
            grad_x = model['L1'].backward(x, my_a1)
