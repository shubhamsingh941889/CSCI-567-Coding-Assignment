...
def model_training(train_data, tags):
    ...
    #   to be zero.
    ###################################################
    my_fstag_cnt = defaultdict(int)
    tag_cnt = defaultdict(int)
    all_trans = defaultdict(int)
    pos_word = defaultdict(lambda: defaultdict(int))
    tag_tans_cnt = defaultdict(lambda: defaultdict(int))

    for sentence in train_data:
        last_tag = ""
        my_fstag_cnt[sentence.tags[0]] = my_fstag_cnt[sentence.tags[0]] + 1
        for i, word in enumerate(sentence.words):
            current_tag = sentence.tags[i]
            tag_cnt[current_tag] += 1
            all_trans[last_tag] += 1
            pos_word[current_tag][word] += 1
            tag_tans_cnt[last_tag][current_tag] += 1
            last_tag = current_tag

    for tag in tags:
        tag_index = tag2idx[tag]
        pi[tag_index] = my_fstag_cnt[tag] / len(train_data)
        for word, count in pos_word[tag].items():
            B[tag_index][word2idx[word]] = count / tag_cnt[tag]
        for next_tag in tags:
            A[tag_index][tag2idx[next_tag]] = tag_tans_cnt[tag][next_tag] / all_trans[tag]
...
def sentence_tagging(test_data, model, tags):
    ...
    ######################################################################
    S = len(tags)
    nxt_avidx = max(model.obs_dict.values()) + 1
    curr_zval = np.full((S, 1), 10 ** -6)
    for sentence in test_data:
        for word in sentence.words:
            if word not in model.obs_dict.keys():
                model.obs_dict[word] = nxt_avidx
                model.B = np.append(model.B, curr_zval, axis=1)
                nxt_avidx += 1
